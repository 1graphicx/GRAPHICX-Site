...
def patch_plugins():
    ...
    targets = [
        r"C:\Program Files\Adobe\Common\Plug-ins\7.0\MediaCore\Red Giant Universe\Universe_Glow_Glow_AE_Fx.aex",
        r"C:\Program Files\Adobe\Common\Plug-ins\7.0\MediaCore\Red Giant Universe\Universe_Utilities_Unmult_Premium_AE_Fx.aex"
    ]
...
def main():
    if len(sys.argv) < 3:
        print('[!] Usage : python MaxonUniverse.py chemin\\vers\\RedGiant.exe chemin\\vers\\MaxonApp.exe')
        sys.exit(1)

    universe_installer = Path(sys.argv[1])
    maxonapp_installer = Path(sys.argv[2])
    for f in [universe_installer, maxonapp_installer]:
        if not f.exists():
            print(f"[!] {f} introuvable")
            sys.exit(1)

    universe_version = extract_version(universe_installer.name)
    maxon_version = extract_version(maxonapp_installer.name)
    app_name = "Maxon Universe"
    output_name = f"{app_name} v{universe_version}"
...
[Files]
Source: "{str(universe_installer)}"; DestDir: "{{tmp}}"; Flags: deleteafterinstall
...
procedure UniverseInstall;
begin
  WizardForm.StatusLabel.Caption := 'Installation de Universe...';
  Exec(ExpandConstant('{{tmp}}\\{universe_installer.name}'), '', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
end;
